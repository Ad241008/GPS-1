...
function filterGPSDataUltraPrecise(position) {
  ...
  // Filtrage strict : seulement les positions très précises
  ...
  // Garder seulement les 8 dernières positions pour analyse temporelle
  if (positionBuffer.length > 8) {
  ...
  if (positionBuffer.length >= 3) {
    ...
    for (let i = 1; i < recent.length; i++) {
      ...
      if (timeDiff > 0.8 && timeDiff < 4 && distance > 0.5) { // Entre 0.8 et 4 secondes, distance > 0.5m
...
function setSpeed(kmh){ 
  if (kmh !== null && Number.isFinite(kmh)) {
    ...
    if (speedDisplayBuffer.length > 3) speedDisplayBuffer.shift();
    
    // Moyenne mobile sur 3 valeurs pour éliminer le "sautillement"
...
function setHeading(degrees) {
  if (degrees !== null && Number.isFinite(degrees)) {
    ...
    // Ne mettre à jour l'affichage que si le changement est significatif
    if (lastDisplayedHeading === null || Math.abs(displayHeading - lastDisplayedHeading) >= 3) {
      headingEl.textContent = `${displayHeading}°`;
      lastDisplayedHeading = displayHeading;
      
      // Mettre à jour le marqueur encore moins fréquemment pour éviter les saccades
      headingUpdateCounter++;
      if (headingUpdateCounter % 3 === 0 && myMarker && myMarker.setIcon) {
...
function startUltraHighAccuracyGeolocation() {
  ...
  function handleUltraPrecisePosition(position) {
    ...
    if (!myMarker) {
      ...
      // PAS de centrage automatique - seulement si l'utilisateur n'a pas bougé la carte
      if (!userHasMovedMap) {
